...
from core.parsing_schemas import ResumeSchema, JobSchema, ScoreBreakdown
...
class JobMatcher:
    ...
    def calculate_skill_match(self, resume: ResumeSchema, job: JobSchema) -> Tuple[float, List[str], List[str]]:
        """
        Calculate skill match score (0-40 points).
        Returns: (score, matched_skills, missing_skills)
        """
        resume_skills = set([s.lower() for s in resume.skills])
        job_requirements = set([r.lower() for r in job.requirements])
        job_nice_to_have = set([n.lower() for n in job.nice_to_have])
    ...
    def calculate_experience_alignment(self, resume: ResumeSchema, job: JobSchema) -> float:
        ...
        resume_domains = set([d.lower() for d in resume.domains])
        job_text_lower = (job.title + " " + job.responsibilities + " ".join(job.requirements)).lower()
    ...
    def calculate_location_language_fit(self, resume: ResumeSchema, job: JobSchema) -> float:
        ...
        if preferred_location_lower in job_location_lower or job_location_lower in preferred_location_lower:
            score += 5
        ...
        resume_languages = set([lang.lower() for lang in resume.languages])
    ...
    def calculate_semantic_similarity(self, resume: ResumeSchema, job: JobSchema) -> float:
        ...
        resume_text = f"{' '.join(resume.skills)} {' '.join(resume.domains)} {' '.join(resume.projects)}"
    ...
    def calculate_match_score(self, resume: ResumeSchema, job: JobSchema) -> Tuple[float, ScoreBreakdown, List[str], List[str]]:
        """
