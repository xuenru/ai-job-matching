...
from core.parsing_schemas import ResumeSchema, JobSchema, RankedJob, RankedJobsOutput, ScoreBreakdown
...
class MatchAgent:
    ...
    def match_jobs(
        ...
        for job in jobs:
            try:
                ...
                score, breakdown, matched_skills, missing_skills = self.matcher.calculate_match_score(
                    resume, job
                )
    ...
    def _generate_explanation(
        ...
        """
        if self.use_llm:
            return self._generate_with_llm(resume, job, score, breakdown, matched_skills, missing_skills)
        else:
            return self._generate_deterministic(resume, job, score, breakdown, matched_skills, missing_skills)
    ...
    def _generate_deterministic(
        ...
        if breakdown.skill_match >= 30:
            ...
            reason_parts.append(f"Limited skill overlap (only {len(matched_skills)} matched skills).")
